import ChairContent from "@/components/ChairContent";
import { fetchDataFromApi } from "@/utils/api";

export default async function Chair() {
  // Fetch all data on the server
  const categories = await fetchDataFromApi("/api/category/search?name=Chair");
  const productData = await fetchDataFromApi("/api/products/filter?category=Chair");
