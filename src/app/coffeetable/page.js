import CoffeeTableContent from "@/components/CoffeeTableContent";
import { fetchDataFromApi } from "@/utils/api";

export default async function Coffeetable() {
  // Fetch all data on the server
  const categories = await fetchDataFromApi("/api/category/search?name=Coffee Table");
  const productData = await fetchDataFromApi("/api/products/filter?category=Coffee Table");
