import StoolContent from "@/components/StoolContent";
import { fetchDataFromApi } from "@/utils/api";

export default async function Stool() {
  // Fetch all data on the server
  const categories = await fetchDataFromApi("/api/category/search?name=Stool");
  const productData = await fetchDataFromApi("/api/products/filter?category=Stool");
...
//               <Image
//                 src={getImage(categoryData[0]?.images[0]) || "/placeholder.jpg"}
//                 alt="Stool Image"
