import DiningTableContent from "@/components/DiningTableContent";
import { fetchDataFromApi } from "@/utils/api";

export default async function DiningTablePage() {
  // Fetch all data on the server
  const categories = await fetchDataFromApi("/api/category/search?name=Dining Table");
  const productData = await fetchDataFromApi("/api/products/filter?category=Dining Table");
