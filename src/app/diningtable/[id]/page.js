...
export default async function DiningDetails({ params }) {
  const { id } = params;

  const product = await fetchDataFromApi(`/api/products/${id}`);
  const edgesRes = await fetchDataFromApi(`/api/productedge/productedges?productId=${id}`);
  const topsRes = await fetchDataFromApi(`/api/producttop/producttops?productId=${id}`);
  const finishesRes = await fetchDataFromApi(`/api/productfinish/productfinishes?productId=${id}`);

  const edges = edgesRes?.edge?.edges || [];
  const tops = topsRes?.top?.tops || [];
  const finishes = finishesRes?.finish?.finishes || [];
  ...
  return (
    <div className="overflow-x-hidden 2xl:container mx-auto">
      ...
      <ProductDetail product={product} edges={edges} tops={tops} finishes={finishes} />
    </div>
  );
}
