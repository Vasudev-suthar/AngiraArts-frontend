...
export default function DiningTableContent({ productData, categories }) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      const res = await fetchDataFromApi(
        `/api/products/filter?category=Dining Table&${queryParams}`
      );
  ...
  const handleOptionClick = (label, value) => {
    if (value === "all") {
      ...
      fetchDataFromApi("/api/products/filter?category=Dining Table").then(
        (res) => {
          setProducts(res.products);
      ...
      <div className="2xl:container my-5 mx-auto px-4 sm:px-16">
        <div className="relative flex mt-3 items-center">
          <div className="w-7/12 flex flex-col justify-center mb-4 xs:mb-0">
            ...
            >
              explore our exquisite collection of dining tables that
              effortlessly combine timeless classic designs with contemporary
