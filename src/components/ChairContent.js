...
export default function ChairContent({ productData, categories }) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      const res = await fetchDataFromApi(
        `/api/products/filter?category=Chair&${queryParams}`
      );
  ...
  const filterOptions = {
    ...
    Category: [
      { _id: "all", name: "Chair" }, // Add "All" option
    ],
    "Remove Filters": [
    ...
    <div className="w-full pb-4 h-auto overflow-x-hidden font-raleway">
      <div className="2xl:container my-5 mx-auto px-4 sm:px-16">
        <div className="relative flex mt-3 items-center">
          <div className="w-8/12 flex flex-col justify-center mb-4 xs:mb-0">
            <motion.div
              className="flex justify-center font-normal lg:font-light text-[20px] sm:text-2xl lg:text-[48px] tracking-[10px] md:tracking-[14px] lg:mb-3"
              initial="offscreen"
            ...
            <motion.p
              className="hidden md:flex text-center font-extralight text-sm lg:text-[18px] mt-2 sm:mt-4 sm:px-2 md:px-8 lg:px-16 tracking-[1.88px] md:tracking-[2.2px] capitalize leading-[1.7]"
              initial="offscreen"
              whileInView="onscreen"
              variants={togVariants}
            >
              chairs that make a statement. from dining spaces to reading nooks,
              find seating that complements your style and support your moments.
            </motion.p>
          </div>
          <div className="w-4/12 aspect-[1.2657] invisible" />
          <motion.div
            className="absolute right-0 w-4/12 flex items-center justify-center lg:justify-end -z-10"
            initial="offscreen"
            whileInView="onscreen"
            variants={slideInRight}
          >
            <div className="relative w-[80%] xs:w-[100%] aspect-[1.3] border-ridge">
