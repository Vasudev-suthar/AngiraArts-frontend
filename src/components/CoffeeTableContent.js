...
export default function CoffeeTableContent({ productData, categories }) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      const res = await fetchDataFromApi(
        `/api/products/filter?category=Coffee Table&${queryParams}`
      );
  ...
  const handleOptionClick = (label, value) => {
    if (value === "all") {
      ...
      fetchDataFromApi("/api/products/filter?category=Coffee Table").then(
        (res) => {
          setProducts(res.products);
      ...
      <div className="2xl:container my-5 mx-auto px-4 sm:px-16">
        <div className="relative flex mt-3 items-center">
          <div className="w-7/12 flex flex-col justify-center mb-4 xs:mb-0">
            ...
            <motion.p
              className="hidden text-center font-extralight md:flex text-sm lg:text-[18px] mt-2 sm:mt-3 sm:px-2 md:px-8 lg:px-6 tracking-[1.88px] md:tracking-[2.2px] capitalize leading-[1.7]"
              initial="offscreen"
              whileInView="onscreen"
              variants={togVariants}
            >
              delve into a curated selection of the finest coffee table designs,
              meticulously crafted to elevate your living space, offering visual
              appeal and functionality that enhances your daily living
              experience.
            </motion.p>
          </div>
